...
`npm run dev` 또는 `npm run build` 안에서 다시 `node`를 찾지 못하는 경우가 있으므로, 로컬 실행 전 PowerShell에 아래 줄을 먼저 입력하는 것을 권장한다. 이 설정은 현재 터미널 창에만 적용된다.
...
만약 현재 폴더가 헷갈리면 아래처럼 실행해도 된다. 이 명령은 어디서 실행해도 `BaroName` 폴더를 기준으로 실행한다.
...
### "node is not recognized" 오류가 나올 때

Node.js는 설치되어 있지만 Windows PATH에 등록되지 않은 상태다. PowerShell에서 아래 줄을 먼저 실행한 뒤 다시 시도한다.
...
### Could not read package.json 오류가 나올 때

예:

```text
Could not read package.json ... C:\Users\JUNE\package.json
```

이 오류는 앱 폴더가 아닌 곳에서 `npm run dev`를 실행했다는 뜻이다. 로그의 `cwd C:\Users\JUNE`가 그 증거다.
...
### Vercel에서 Generate를 눌러도 후보가 안 나올 때

먼저 배포된 사이트 주소 뒤에 `/api/health`를 붙여 접속한다.
...
- `hasGeminiKey`가 `false`이면 Vercel Environment Variables에 `GEMINI_API_KEY`가 빠진 것이다.
- `unlocked`가 `false`이면 사이트에서 비밀번호를 다시 입력한다.
- 모델 오류가 나면 Vercel의 `GEMINI_MODEL` 값을 AI Studio에서 사용 가능한 모델명으로 바꾼다.
- 환경변수를 바꾼 뒤에는 Vercel에서 반드시 Redeploy를 해야 한다.
